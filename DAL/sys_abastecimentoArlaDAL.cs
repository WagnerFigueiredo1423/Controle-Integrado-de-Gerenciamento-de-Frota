...
namespace DAL
{
    public static class sys_abastecimentoArlaDAL
    {
        ...
        public static void InserirDAL(sys_abastecimentosMDL mdlLocal)
        {
            ...
            try
            {
                sqlCom = new MySqlCommand("INSERT INTO " + dbName + ".sys_abastecimentos_arla (id,sys_veiculos_id,data,litros,km,criado,modificado,observacao) VALUES (@ID,@SYS_VEICULOS_ID,@DATA,@LITROS,@KM,@CRIADO,@MODIFICADO,@OBSERVACAO);", con);
        ...
        public static void AtualizarDAL(sys_abastecimentosMDL mdlLocal)
        {
            ...
            try
            {
                sqlCom = new MySqlCommand("UPDATE " + dbName + ".sys_abastecimentos_arla SET id = @ID,sys_veiculos_id = @SYS_VEICULOS_ID,data = @DATA,litros = @LITROS,km = @KM,modificado = @MODIFICADO,observacao = @OBSERVACAO WHERE id = @ID;", con);
        ...
        public static void DeletarDAL(int id)
        {
            ...
            try
            {
                sqlCom = new MySqlCommand("DELETE FROM " + dbName + ".sys_abastecimentos_arla WHERE id = " + id + ";", con);
                con.Open();
                sqlCom.ExecuteNonQuery();
            }
            catch (MySqlException erro)
        ...
        public static DataTable ListarDAL(string tipo, string indexPlaca, DateTime data)
        {
            ...
            try
            {
                ...
                {
                    case "tudo":
                        sqlCom = new MySqlCommand("SELECT data,litros,km FROM " + dbName + ".sys_abastecimentos_arla ORDER BY data DESC;", con);
                        break;
                    case "placa":
                        sqlCom = new MySqlCommand("SELECT data,litros,km FROM " + dbName + ".sys_abastecimentos_arla WHERE sys_veiculos_id =  " + indexPlaca + " ORDER BY data DESC;", con);
                        break;
                    case "plData":
                        sqlCom = new MySqlCommand("SELECT sys_abastecimentos_arla.id, data, litros, km FROM " + dbName + ".sys_abastecimentos_arla," + dbName + ".sys_veiculos WHERE sys_veiculos.id = sys_veiculos_id AND sys_veiculos_id = " + indexPlaca + " AND (MONTH(data) = " + data.Month + ") AND (YEAR(data) = " + data.Year + ") ORDER BY data ASC;", con);
                        break;
                }
        ...
        public static DataTable ListarRelatorioDAL(string tipo, string indexPlaca, DateTime data)
        {
            ...
            try
            {
                ...
                {
                    case "comparativo":
                        sqlCom = new MySqlCommand("SELECT sys_abastecimentos_arla.id, data,litros,km FROM " + dbName + ".sys_abastecimentos_arla," + dbName + ".sys_veiculos WHERE sys_veiculos.id = sys_abastecimentos.sys_veiculos_id AND sys_veiculos.id = " + indexPlaca + "  AND (MONTH(data) = " + data.Month + ") AND (YEAR(data) = " + data.Year + ") ORDER BY data ASC;", con);
                        break;
                    case "evolutivo":
                        sqlCom = new MySqlCommand("SELECT data, litros, km FROM " + dbName + ".sys_abastecimentos_arla WHERE sys_veiculos_id =  " + indexPlaca + " ORDER BY data DESC;", con);
                        break;
